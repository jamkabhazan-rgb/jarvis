...
| 2 | **Chat + LLM (text)** | Orchestrator, OpenAI chat-completions with `tools`, token streaming to the chat UI, tool-call deltas. | **working** — real streaming chat-completions in the core, key in OS keychain, chat UI wired via events, **conversation memory** (rolling history per chat + per agent), retries with backoff on 429/5xx |
...
| 5 | **Knowledge vault** | Markdown vault (Obsidian-compatible, YAML frontmatter, `[[wikilinks]]`) + embeddings semantic search. | **in progress** — notes CRUD, full-text search, tag filter, `[[wikilinks]]` + backlink counts; **long-term memory** (durable facts saved via `memory_save`, injected into every conversation's system prompt, managed in Settings) (localStorage); markdown-file sync + embeddings next |
